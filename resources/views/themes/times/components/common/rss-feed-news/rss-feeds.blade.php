<section class="container my-6">
    ...
    <div
        ...
        <h1 class="text-blue-600 dark:text-white uppercase text-lg md:text-xl font-bold">
    ...
    <div id="tab-content" class="relative mt-6">
        @foreach ($rssFeedNews as $rssFeedNewsData)
            @if ($loop->first)
                @include('themes.times.components.common.rss-feed-news.tab-one', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @else
                @include('themes.times.components.common.rss-feed-news.tab-news', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @endif
        @endforeach
