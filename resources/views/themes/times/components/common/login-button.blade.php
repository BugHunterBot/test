    ...
    @if (Auth::guard('webuser')->check())
        ...
        <div class="relative">
            <button type="button" class="profile_toggle flex items-center gap-2">
                <figure
                    class="w-12 h-12 lg:w-9 lg:h-9 bg-neutral-100 dark:bg-theme-three flex_center rounded-md lg:rounded-full overflow-hidden relative border dark:border-neutral-800">
                    @if ($hasImage)
                        <img class="w-full h-full object-cover" src="{{ asset('storage/' . $user->avatar) }}"
                            alt="user profile image" />
                    @else
                        <span
                            class="text-white text-sm font-semibold w-full h-full flex items-center justify-center">
                            {{ $initials }}
                        </span>
                    @endif
                </figure>
    ...
    @else
        ...
        <div class="flex divide-x rtl:divide-x-reverse">
            <button type="button" id="openModal"
                class="px-2 text-white hover:text-blue-400 dark:hover:text-blue-400 transition_3">
                {{ localize('login') }}
            </button>
            <a href="{{ __url('registration') }}"
                class="pl-2 rtl:pl-0 rtl:pr-2 text-white hover:text-blue-400 dark:hover:text-blue-400 transition_3">
