<section class="container my-6">
    ...
    <div
        ...
        <h1 class="text-theme-five-primary dark:text-white text-lg font-semibold uppercase hover:text-brand-primary">
    ...
    <div id="tab-content" class="relative mt-6">
        @foreach ($rssFeedNews as $rssFeedNewsData)
            @if ($loop->first)
                @include('themes.gazette.components.common.rss-feed-news.tab-one', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @else
                @include('themes.gazette.components.common.rss-feed-news.tab-news', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @endif
        @endforeach
