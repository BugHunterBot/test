<section class="container my-8 xl:my-12">

    <!-- section head -->
    <div class="md:flex justify-between gap-4 items-center mb-4">
        <h1
        ...
        <ul id="tabs" class="flex gap-4 md:gap-6 text-base text-neutral-500 cursor-pointer">
            <li class="tab_gazette1 tab_nav_link" data-tab="all">{{ localize('all') }}</li>
            @if ($sectionTwoAllSubNews->isNotEmpty())
                @foreach ($sectionTwoAllSubNews as $data)
                    <li class="tab_gazette1" data-tab="{{ $data['subcategory']->slug }}">
    ...
    <div id="tab-content" class="relative mt-6">
        <!-- All Category  -->
        @include('themes.gazette.components.home.tab-1.all-tab-content')

        <!-- Sub Category  -->
        @if ($sectionTwoAllSubNews->isNotEmpty())
            @foreach ($sectionTwoAllSubNews as $secTwoSubData)
                @include('themes.gazette.components.home.tab-1.sub-tab-content', [
                    'secTwoSubData' => $secTwoSubData,
                ])
            @endforeach
        @endif
    </div>
