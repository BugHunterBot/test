...
<div
    ...
    <ul id="tabs" class="flex gap-4 md:gap-6 text-base text-neutral-500 cursor-pointer">
        <li class="tab_gazette3 tab_nav_link" data-tab="all">{{ localize('all') }}</li>
        @if ($sectionThreeAllSubNews->isNotEmpty())
            @foreach ($sectionThreeAllSubNews as $data)
                <li class="tab_gazette3" data-tab="{{ $data['subcategory']->slug }}">
...
<div id="tab-content" class="relative mt-6">
    <!-- All Category  -->
    @include('themes.gazette.components.home.tab-3.all-news')

    <!-- Sub Category  -->
    @if ($sectionThreeAllSubNews->isNotEmpty())
        @foreach ($sectionThreeAllSubNews as $secThreeSubData)
            @include('themes.gazette.components.home.tab-3.sub-tab-content', [
