<section class="md:border dark:border-neutral-800 md:p-2 md:rounded-md">
    <div class="flex justify-between items-center gap-2 mb-4">
        ...
        <div class="flex items-center gap-2">
            <button type="button"
                class="bg-red-300 hover:bg-brand-primary w-6 h-6 rounded-sm flex justify-center items-center transition-all duration-300 ease-in-out tech-news-prev rtl:order-2">
            ...
            <button type="button"
                class="bg-red-300 hover:bg-brand-primary w-6 h-6 rounded-sm flex justify-center items-center transition-all duration-300 ease-in-out tech-news-next">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                    viewBox="0 0 20 20" fill="none">
                    <path
    ...
    <!-- Slider Section -->
    <div class="swiper w-full h-full TechNewsSwiper">

        <!-- Loader Slide - already in DOM -->
        <div id="sec-five-first-loader" class="flex justify-center items-center hidden">
            <div class="w-10 h-10 border-4 border-t-transparent border-blue-500 rounded-full animate-spin"></div>
        </div>

        <div class="swiper-wrapper" id="append-next-swiper-slide-sec-five-first">

            <!-- Slider 1 -->
            <div class="swiper-slide" id="swiper-slide-sec-five-first-1">
                <a href="{{ __url($sectionFiveNews['firstNews'][0]->news->encode_title) }}" class="bg_gradient_top block">
                    ...
                    <div class="p-2 2xl:p-3 absolute z-10 bottom-0 left-0">
                        ...
                        <div class="text-white capitalize flex items-center gap-1 text-xs">
                            ...
                            <span>{{ $sectionFiveNews['firstNews'][0]->news->comments_count }}</span>
                        </div>
                    </div>
                </a>
                ...
                <div class="space-y-2 divide-y divide-neutral-200 dark:divide-neutral-700 mt-2">
                    <!-- bottom news list -->
                    @if ($sectionFiveNews['firstNews']->slice(1)->isNotEmpty())

                        @foreach ($sectionFiveNews['firstNews']->slice(1) as $secFiveFirstNewsList)
                            <a class="block pt-2" href="{{ __url($secFiveFirstNewsList->news->encode_title) }}">
                                <div class="grid grid-cols-3 gap-2 items-center">
                                    <figure class="w-full h-20">
                                        <img class="w-full h-full object-cover"
                                            src="{{ isset($secFiveFirstNewsList->news->photoLibrary->image_base_url) ? $secFiveFirstNewsList->news->photoLibrary->image_base_url : asset('/assets/opinion-avatar.png') }}"
                                            alt="{{ $secFiveFirstNewsList->news->image_alt }}" />
                                    </figure>
                                    <h2
                                        ...
                                        {{ $secFiveFirstNewsList->news->title }}
                                    </h2>
                                </div>
                            </a>
                        @endforeach
            ...
            <div class="swiper-slide" id="no-need-to-slide-sec-five-first">
            </div>
