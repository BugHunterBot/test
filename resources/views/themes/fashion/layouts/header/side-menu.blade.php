    ...
    <div class="side-bar bg-[#1b060e] dark:bg-[#0e0307] h-full p-6 pt-2 overflow-x-hidden overflow-y-auto shadow-md side_bar" id="sidebar">
        ...
        @if ($languages->count() > 1)
            <div class="relative inline-block mt-6">
                ...
                <ul id="dropdownMenu"
                    class="absolute left-0 rtl:right-0 rtl:left-auto mt-1 w-max bg-[#000000] dark:bg-[#1b060e] rounded-sm shadow-md overflow-hidden hidden z-50">
                    @foreach ($languages as $lang)
                        <li>
                            <a href="javascript:void(0)" data-lang="{{ $lang->value }}"
                                class="language_side block px-4 py-2 focus:bg-theme-six-secondary/60 focus:text-white
                                {{ $lang->value == app()->getLocale() ? 'bg-theme-six-secondary text-white' : 'text-white' }}">
                                {{ $lang->langname }}
                            </a>
                        </li>
                    @endforeach
