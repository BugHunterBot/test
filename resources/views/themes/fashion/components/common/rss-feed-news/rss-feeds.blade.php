<section class="container my-6">
    ...
    <div
        ...
        <h1 class="text-theme-six-primary text-lg md:text-xl font-bold uppercase">
    ...
    <div id="tab-content" class="relative mt-6">
        @foreach ($rssFeedNews as $rssFeedNewsData)
            @if ($loop->first)
                @include('themes.fashion.components.common.rss-feed-news.tab-one', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @else
                @include('themes.fashion.components.common.rss-feed-news.tab-news', [
                    'rssTabData' => $rssFeedNewsData,
                ])
            @endif
        @endforeach
