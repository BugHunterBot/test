<section
  ...
  <div class="space-y-4 mt-4">
        @foreach ($popularNews->slice(0, 2) as $popularNewsItem)
            <div class="overflow-hidden">
                <a href="{{ __url($popularNewsItem->encode_title) }}" class="w-full h-48 group overflow-hidden block">
                ...
                <div class="p-2 space-y-4">
                    <div class="line-clamp-2">
                        <a href="{{ __url($popularNewsItem->encode_title) }}"
                            class="text-theme-six-secondary hover:text-theme-six-primary dark:text-neutral-50 dark:hover:text-theme-six-primary font-bold text-lg leading-6 transition_3 ">
                            {{ $popularNewsItem->title }}
                        </a>
                    </div>

                    <div class="text-neutral-500 capitalize flex items-center gap-1 text-sm">
                        <span>{{ $popularNewsItem->postByUser->full_name ?? localize('unknown') }}</span>
                        ...
                        <span>{{ $popularNewsItem->comments_count }}</span>
                    </div>
                </div>
            </div>
        @endforeach
