...
<div class="col-xl-6">
    <div class="card shadow-1">
      <div class="card-header">
        <div class="align-items-center d-flex flex-wrap gap-2 justify-content-between">
          <div>
            <h6 class="fs-16 fw-bold mb-0">Less Selling Product</h6>
          </div>
          <div class="text-end">
            <div id="lesssellingrange" class="d-flex align-items-center predefined bg-white-smoke border-0">
              ...
              <span id="lesssellingDates" class="date-arange"></span>
            </div>
          </div>
        </div>
      </div>
      {{-- <div class="card-body" id="lessSellingProduct" data-url="{{route('lessSellingProduct')}}"> --}}
      <div class="card-body">
  ...
  <script>
    ...
    var start = moment().subtract(29, "days");
        ...
        function newDateRHMJ(start, end) {
            $("#lesssellingrange span").html(start.format("MMM D, YYYY") + " - " + end.format("MMM D, YYYY"));
        }
        $("#lesssellingrange").daterangepicker({
                startDate: start,
                endDate: end,
                showDropdowns: true,
                ranges: {
        ...
        newDateRHMJ(start, end);

    // Update chart there
    function updateChartNine(dateRange) {
        var url  = $('#lessSellingProduct').data('url');
    ...
    $(document).ready(function() {
            updateChartNine($('#lesssellingDates').text());
            // Handle date range change event
            $('#lesssellingrange').on('apply.daterangepicker', function(ev, picker) {
                var startDate = picker.startDate.format('YYYY-MM-DD');
                var endDate = picker.endDate.format('YYYY-MM-DD');
                var dateRange = startDate + ' - ' + endDate;
                updateChartNine(dateRange);
            });
        });
</script>
