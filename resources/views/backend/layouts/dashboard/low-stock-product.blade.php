    ...
    <div class="card shadow-1">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <h6 class="fs-16 fw-bold mb-0">Low Stock Alert</h6>
          </div>
          <div class="text-end overflow-auto">
            <div id="lowstockrange" class="predefined bg-white-smoke d-flex align-items-center border-0">
      ...
      <div class="card-body">
        <div class="table-responsive overflow-hidden">
            {{ $LowStockProduct->table()}}
        </div>
      </div>
    </div>
  </div>
@push('js')
{!! $LowStockProduct->scripts() !!}
    <script>
        ...
        function lsr(start, end) {
            $("#lowstockrange span").html(start.format("MMM D, YYYY") + " - " + end.format("MMM D, YYYY"));
        }

        $("#lowstockrange").daterangepicker(
            {
                startDate: start,
                endDate: end,
                showDropdowns: true,
        ...
        //on change date range
            $('#lowstockrange').on('apply.daterangepicker', function(ev, picker) {
                var start = picker.startDate.format('YYYY-MM-DD');
                var end = picker.endDate.format('YYYY-MM-DD');

                var table = $('#low-stock-alert-product-table');
