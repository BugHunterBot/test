            ...
            <div class="col-md-12 col-lg-12">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-12 d-flex justify-content-between align-items-center mb-5 p-3 bg-soft-aliceblue">
                            <form action="{{ route('setting.language.import') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="locale" value="{{ $lanfolder }}">

                                <label for="file" class="col-form-label fs-20">
                                    {{ localize('upload_translated_excel_file') }}
                                    <input type="file" class="form-control" name="file" id="file" accept=".xlsx" required>
                                </label>

                                <button type="submit" class="btn btn-success">{{ localize('upload') }}</button>
                            </form>
                            
                            <a href="{{ route('setting.language.export', ['locale' => $lanfolder]) }}" class="btn btn-primary">    
                                {{ localize('download_excel') }}
                            </a>
                        </div>
