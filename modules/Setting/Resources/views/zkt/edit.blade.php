<div class="card-body p-1">
    <div class="row mb-2">
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="device_id" id="device_id" value="{{ $data->device_id }}" />
    ...
    <div class="row mb-2">
        <div class="form-group">
            <label for="status" class="col-form-label fw-semibold">{{ localize('status') }}<span
                    class="text-danger">*</span></label>
            <select name="status" class="select-basic-single" id="machine_status">
                <option value="" disabled> {{ localize('select_one') }} </option>
                <option value="1" {{ $data->status == 1 ? 'selected' : '' }}>{{ localize('active') }}</option>
                <option value="0" {{ $data->status == 0 ? 'selected' : '' }}>{{ localize('inactive') }}</option>
            </select>
