            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                <select name="theme" id="theme"  class="form-control" onchange="loadPagePositions()" required>
                    ...
                    @foreach ($themes as $theme)
                        <option value="{{ $theme->id }}" {{ $theme->is_active == 1 ? 'selected' : '' }}>
                            {{ $theme->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="col-md-12 mt-3">
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                ...
                <select name="page" id="page" class="form-control" onchange="loadPagePositions()" required>
                    <option value="">{{localize('select')}}</option>
                    <option value="1">{{makeString(['home_page'])}}</option>
                    <option value="2">{{makeString(['category','page'])}}</option>
                    <option value="3">{{makeString(['news','details','page'])}}</option>
                </select>
        ...
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                ...
                <select name="ad_type" id="ad_type" class="form-control" onchange="ad_type_change(this.value)" required>
                    <option value="">{{localize('select')}}</option>
                    <option value="1">{{makeString(['script','ads'])}}</option>
        ...
        <div class="row">
            ...
            <div class="col-sm-9 col-md-12 col-xl-9">
                <textarea class="form-control" name="embed_code" id="embed_code" rows="5"
                    placeholder='<script async src="https://..."></script>(script + ins + script)'></textarea>
            </div>
