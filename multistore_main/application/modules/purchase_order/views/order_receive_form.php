                            ...
                            <table class="table table-bordered table-hover" id="purchase">
                                ...
                                <tbody id="addinvoiceItem">
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="text" name="product_code[]" value="<?php echo $receive['product_code']; ?>" class="form-control " required placeholder='<?php echo display('product_code') ?>' id="product_c" tabindex="4" readonly=""></td>
                                        <td><input type="text" name="store_name" value="<?php echo $receive['store_name']; ?>" class="form-control " placeholder='<?php echo display('store_name') ?>' id="store_name" tabindex="5" readonly="" required>
                                            <input type="hidden" name="store_id[]" value="<?php echo $receive['store_id']; ?>" class="form-control" required></td>
                                        <td>
                                            <input type="text" name="recv_qty[]" class="form-control text-right " id="orderqty_<?php echo $sl;?>" value="<?php

                                   $total_order = $this->db->select('sum(order_qty) as oqty')->from('purchase_order_details')->where('po_no',$order->po_no)->where('product_id',$receive['product_id'])->get()->row();
                                $total_receive = $this->db->select('purchase_receive.*,sum(purchase_receive_details.receive_qty) as rqty')
                                                          ...
                                                          ->where('product_id',$receive['product_id'])
                                                          ->get()
                                                          ->row();
                                                            $total_orde = $total_order->oqty;
                                                          $total_receive = $total_receive->rqty;
                                                         echo  $diff = $total_orde - $total_receive;?>" readonly="" tabindex="6" />
                                      </td>
                                      <td>
                                           <input type="text" name="product_quantity[]" id="quantity_<?php echo $sl;?>" class="form-control text-right store_cal_<?php echo $sl;?>" onkeyup="calculate_store(<?php echo $sl;?>),checkqty(<?php echo $sl;?>);" onchange="calculate_store(<?php echo $sl;?>);" placeholder="0" value="" min="0" tabindex="7"/>
                                        </td>
                                         <td class="text-right">
                                                <input type="text" name="um[]" id="um_<?php echo $sl;?>" class="form-control text-center" tabindex="8" required="required" value="<?php echo $receive['unit_name']; ?>" readonly="readonly"/>
                                            </td>
                                       
                                        <td>
                                              <input type="text" name="product_rate[]" onkeyup="calculate_store(<?php echo $sl;?>),checkqty(<?php echo $sl;?>);" onchange="calculate_store(<?php echo $sl;?>);" id="product_rate_<?php echo $sl;?>" class="form-control product_rate_<?php echo $sl;?> text-right" placeholder="0" value="<?php echo $receive['product_rate'];?>" min="0" tabindex="9" required <?php if ($this->session->userdata('is_admin')!=1) echo 'readonly="readonly"';?> />
                                        </td>
                                        <td class="test">
                                                <input type="text" name="discount[]" onkeyup="calculate_store(<?php echo $sl;?>),checkqty(<?php echo $sl;?>);" id="discount_<?php echo $sl;?>" class="form-control discount_<?php echo $sl;?> text-right" placeholder="0.00" value="" min="0" tabindex="10"/>
                                            </td>
                                        <!-- Discount -->
                                        <td>
