            ...
            <div class="col-sm-12">
                <div class="panel panel-default">
                    <div class="panel-body"> 
                        <?php echo form_open('stock/stock/stock_history_report',array('class' => 'form-inline'))?>
         ...
         echo "Phone :".$companyinfo->phone;
            ...
            $stock = $this->db->select('purchase_receive_details.*,sum(purchase_receive_details.receive_qty) as total_purchase,product.*')
                ...
                <table width="100%" class="table table-striped table-bordered table-hover">
                    <caption><center><h2><?php echo $stocstore->store_name;?></h2></center></caption>
                    <thead>
                        <tr>
                       ...
                       <?php $sl=1;
                        foreach ($stock as $stockinfo) {?>
                                ...
                                <tr class="<?php echo ($sl & 1)?"odd gradeX":"even gradeC" ?>">
                                    ...
                                    <td><?php 
 $totalsale = $this->db->select('sales_parent.*,sale_details.qty,sum(sale_details.qty) as total_sale')
                       ->from('sale_details')
                       ...
                       ->get()
                       ->row();
                       echo $total_sale= $totalsale->total_sale;
...
$movein = $this->db->select('sum(stock_movement_details.received_qty) as move_in,stock_movement.*')
                       ...
                       ->get()
                       ->row();
 $moveout = $this->db->select('sum(stock_movement_details.received_qty) as move_out,stock_movement.*')
                       ->from('stock_movement_details')
                       ->join('stock_movement','stock_movement_details.movement_id=stock_movement.movement_id')
                  ...
                  echo $total_pur_ret=$purreturn->toal_pur_return;
                       ...
                       echo $total_sale_retu = $salesreturn->total_sales_return;
                                     ?></td>
                                     <td><?php  $in = $total_purchase+$total_sale_retu+$movein_total;
                                     $out =  $totalsale+$purreturn+$moveout_total;
                                     echo $in-$out; ?></td>
                                </tr>
                             <?php $sl++; } ?>
