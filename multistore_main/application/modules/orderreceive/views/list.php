                    ...
                    <table class="datatable2 table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <?php foreach ($purchase_orders as $order) { 
                             $total_order = $this->db->select('count(row_id) as orders')->from('purchase_order_details')->where('po_no',$order->po_no)->get()->row();
                                $total_receive = $this->db->select('purchase_receive.*,count(purchase_receive_details.receive_id) as receive')
                            ...
                            <tr>
                                ...
                                <?php if($order->isapproved == 1 Or $this->session->userdata('isAdmin')) { ?>
                                 ...
                                 if($this->session->userdata('isAdmin') Or $role['roleid'] == 2) {
                              
                                 ?>
                                 <?php if($diff > 0 OR $this->session->userdata('isAdmin'))  {?>
